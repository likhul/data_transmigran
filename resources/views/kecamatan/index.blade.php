...
@section('title', 'Master Kecamatan - SI Jambi')
...
<div class="container-fluid py-3">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
        ...
        <button class="btn-premium btn-blue shadow" data-bs-toggle="modal" data-bs-target="#modalTambah">
            <i class="bi bi-plus-circle-fill fs-5"></i> Tambah Kecamatan
        </button>
    </div>
    ...
    <div class="premium-card mb-5">
        ...
        <div class="p-4 border-bottom border-light">
            <form action="{{ route('kecamatan.index') }}" method="GET" class="w-100">
                <div class="search-pill">
                    <i class="bi bi-search text-muted fs-5"></i>
                    <input type="text" name="search" placeholder="Cari kecamatan atau kabupaten..." value="{{ request('search') }}">
        ...
        <div class="table-responsive">
            <table class="table table-modern">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="text-center text-muted fw-bold">{{ $kecamatans->firstItem() + $index }}</td>
                        <td>
                            <div class="fw-bold text-dark" style="font-size: 1rem;"><i class="bi bi-geo-alt text-muted me-2"></i>{{ $k->nama_kecamatan }}</div>
                        </td>
                        <td>
                            <span class="soft-badge"><i class="bi bi-map me-1"></i>{{ $k->kabupaten->nama_kabupaten ?? '-' }}</span>
                        </td>
                        <td class="text-center">
                            <div class="d-flex justify-content-center gap-2">
                                ...
                                <button type="button" class="action-btn delete btn-hapus-sweet" 
                                        data-id="{{ $k->id }}" 
                                        data-nama="{{ $k->nama_kecamatan }}" title="Hapus">
                    ...
                    <tr>
                        <td colspan="4" class="text-center py-5">
                            <div class="mb-3"><i class="bi bi-geo-alt text-slate-200" style="font-size: 3rem;"></i></div>
                            <h6 class="fw-bold text-slate-700">Belum Ada Data Kecamatan</h6>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div class="px-4 py-3 border-top border-light d-flex justify-content-between align-items-center bg-white">
            <span class="text-muted small fw-bold">Menampilkan {{ $kecamatans->firstItem() }} - {{ $kecamatans->lastItem() }}</span>
            <div class="pagination-sm m-0">{{ $kecamatans->links('pagination::bootstrap-5') }}</div>
        </div>
        @endif
    </div>
</div>
...
<div class="modal fade" id="modalEdit{{ $k->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form action="{{ route('kecamatan.update', $k->id) }}" method="POST" class="modal-content" style="border-radius: 24px; border: none; box-shadow: 0 20px 40px rgba(0,0,0,0.1);">
            @csrf @method('PUT')
            <div class="modal-header border-0 p-4 pb-0">
                <h5 class="fw-bold text-dark">Edit Data Kecamatan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-4">
                <div class="mb-3">
                    <label class="form-label fw-bold small text-muted text-uppercase">Pilih Kabupaten</label>
                    <select name="kabupaten_id" class="form-select form-control-premium" required>
                        @foreach($kabupatens as $kb)
                            <option value="{{ $kb->id }}" {{ $k->kabupaten_id == $kb->id ? 'selected' : '' }}>{{ $kb->nama_kabupaten }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-3">
                    <label class="form-label fw-bold small text-muted text-uppercase">Nama Kecamatan</label>
                    <input type="text" name="nama_kecamatan" class="form-control form-control-premium" value="{{ $k->nama_kecamatan }}" required>
                </div>
...
<div class="modal fade" id="modalTambah" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form action="{{ route('kecamatan.store') }}" method="POST" class="modal-content" style="border-radius: 24px; border: none; box-shadow: 0 20px 40px rgba(0,0,0,0.1);">
            @csrf
            <div class="modal-header border-0 p-4 pb-0">
                <h5 class="fw-bold text-dark">Tambah Kecamatan Baru</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-4">
                <div class="mb-3">
                    ...
                    <select name="kabupaten_id" class="form-select form-control-premium" required>
                        ...
                        @foreach($kabupatens as $kb)
                            <option value="{{ $kb->id }}">{{ $kb->nama_kabupaten }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-3">
                    <label class="form-label fw-bold small text-muted text-uppercase">Nama Kecamatan</label>
                    <input type="text" name="nama_kecamatan" class="form-control form-control-premium" placeholder="Misal: Rantau Rasau" required>
                </div>
    ...
    $(document).ready(function() {
        $('.btn-hapus-sweet').on('click', function() {
            ...
            Swal.fire({
                ...
                html: `Apakah Anda yakin ingin menghapus <b>${nama}</b>?<br><span class="text-danger small">Tindakan ini akan gagal jika masih ada data UPTD yang terdaftar di kecamatan ini.</span>`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    let formAction = "{{ route('kecamatan.destroy', ':id') }}".replace(':id', id);
